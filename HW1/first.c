...
int sort_student(char *file_name, char *option, char *order){
    ...
    // Create a StudentGrade array
    StudentGrade **student_grades = (StudentGrade **)malloc(total_size * sizeof(StudentGrade *));
    ...
    while ((n = read(fd, buffer, sizeof(buffer) - 1)) > 0) {
        for (int i = 0; i < n; i++) {
            ...
            if (c == '\n' || c == '\0') {
                ...
                // Add the student id and grade to the StudentGrade array
                student_grades[num_of_lines] = (StudentGrade *)malloc(sizeof(StudentGrade));
                student_grades[num_of_lines]->student_id = strdup(student_id);
                student_grades[num_of_lines]->grade = strdup(grade);

                num_of_lines++;

                // make bigger the array if it is full (twice the size each time)
                if (num_of_lines == 100){
                    total_size *= 2;
                    student_grades = (StudentGrade **)realloc(student_grades,total_size * sizeof(StudentGrade *));
                }
